...
class _HomeScreenState extends State<HomeScreen> {
    ...
    try {
      ...
      if (meRes.statusCode == 200) {
        ...
        if (travelRes.statusCode == 200) {
          ...
          final parsedTrips =
              travelList.map((json) => TravelModel.fromJson(json)).toList();

          final now = DateTime.now();
          final futureTrips = parsedTrips
              .where(
                  (t) => t.startDate.isAfter(now) || t.startDate.day == now.day)
              .toList();
      ...
      if (friendRes.statusCode == 200) {
        ...
        setState(() {
          _friends =
              friendList.map((json) => FriendHomeModel.fromJson(json)).toList();
        });
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
          ? const Center(
              child: CircularProgressIndicator(color: Color(0xFF4E48AF)))
          : RefreshIndicator(
                    ...
                    child: Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          const Row(
                            children: [
                              Icon(Icons.people_alt_rounded,
                                  color: Color(0xFF4E48AF), size: 20),
                              SizedBox(width: 8),
                              Text('친구 목록',
                                  style: TextStyle(
                                      fontSize: 17,
                          ...
                          GestureDetector(
                            onTapDown: (_) =>
                                setState(() => _addBtnScale = 0.92),
                            onTapUp: (_) => setState(() => _addBtnScale = 1.0),
                            onTapCancel: () =>
                                setState(() => _addBtnScale = 1.0),
                ...
                children: [
                  Row(
                    children: [
                      ...
                      Text(widget.myUniqueId,
                          style: const TextStyle(
                              fontSize: 13,
                              fontWeight: FontWeight.bold,
                              color: Color(0xFF4E48AF),
                              letterSpacing: 0.5)),
                    ],
                  ),
                  GestureDetector(
                    onTap: () {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                          content: Text('고유 ID가 클립보드에 복사되었습니다.')));
                    },
                    child: const Icon(Icons.copy,
                        size: 16, color: Color(0xFF4E48AF)),
                  ),
                ],
              ),
            ),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                        backgroundColor: const Color(0xFF4E48AF),
                ...
                child: Row(
                  children: [
                    CircleAvatar(
                        ...
                        backgroundColor: const Color(0xFF4E48AF),
                        child: Text(
                            _searchedNickname!.substring(
                                0, _searchedNickname!.length >= 2 ? 2 : 1),
                            style: const TextStyle(
                                color: Colors.white,
                                fontSize: 13,
                                fontWeight: FontWeight.bold))),
                    ...
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(_searchedNickname!,
                            style: const TextStyle(
                                fontSize: 14,
                                fontWeight: FontWeight.bold,
                                color: Color(0xFF1E2939))),
                        const SizedBox(height: 2),
                        Text('ID: $_searchedUniqueId',
                            style: const TextStyle(
                ...
                child: AnimatedScale(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(
                            color: const Color(0xFF4E48AF).withOpacity(0.25),
                            blurRadius: 8,
          ...
          Row(
            ...
            children: [
              ...
              Container(
                ...
                child: Text(trip.dDay == 0 ? 'D-Day' : 'D-${trip.dDay}',
                    style: const TextStyle(
                        fontSize: 11,
                        fontWeight: FontWeight.w700,
                        color: Colors.white)),
...
class _FriendListItemWidget extends StatelessWidget {
  ...
  Future<void> _deleteFriendAction(BuildContext context) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('친구 삭제 처리에 실패했습니다. (코드: ${response.statusCode})')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    final avatarText =
        avatarBg.computeLuminance() > 0.5 ? Colors.black87 : Colors.white;

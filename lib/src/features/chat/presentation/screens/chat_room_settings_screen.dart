...
class _ChatRoomSettingsScreenState extends State<ChatRoomSettingsScreen> {
  ...
  Future<void> _updateRoomTitleOnServer(String newName) async {
    try {
      ...
      final targetUrl =
          '${AuthStorage.baseUrl}/chat/${widget.roomId}/name?room_name=$newName';
      final response = await http.put(Uri.parse(targetUrl),
          headers: AuthStorage.authHeaders);
  ...
  Future<void> _uploadRoomPhotoSimulate() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
          content: Text('📸 갤러리 연동 및 대표 이미지 업로드를 시작합니다.',
        ...
        children: [
          ...
          Center(
            child: Stack(
              children: [
                const CircleAvatar(
                    radius: 50,
                    backgroundColor: Color(0xFF925DFB),
            ...
            child: Column(
              children: widget.activeMemberIds.map((id) {
                ...
                final String shortName =
                    memberName.isNotEmpty ? memberName.substring(0, 1) : '대';
          ...
          _buildSettingTile(Icons.person_add_alt, '친구 초대', () {
            Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (_) => FriendInviteScreen(roomId: widget.roomId)));
          }),
